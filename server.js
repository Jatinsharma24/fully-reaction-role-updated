...
client.on("message", async message => {
  ...
  switch (cmd.toLowerCase()) {
    case "purge":
      if (!args[0])
        return message.channel.send(`Please provide an amount of messages`);
      if (isNaN(args[0]))
        return message.channel.send(
          `The amount of messages need to be numeric value.`
        );
      let amount = parseInt(args[0]);
      if (amount === 0)
        return message.channel.send(`The amount of messages cannot be 0`);
      ...
      await message.channel.messages.fetch({ limit: amount }).then(msgs => {
        message.channel
          ...
          .catch(() =>
            message.channel.send(`I cannot delete messages more than 2 weeks.`)
          );
      });
      break;
    case "prefix":
      message.channel.send(
        `The prefix in this server is: \`${prefixes[message.guild.id]}\``
      );
      break;
    case "setprefix":
      ...
      if (!args[0])
        return message.channel.send(
          `Please provide a prefix. (Current prefix: ${
      ...
      message.channel.send(
        `The current prefix in this server is now: \`${args.join(" ")}\``
      );
      break;
    case "help":
    ...
    case "create":
      ...
      if (!message.guild.me.hasPermission("MANAGE_CHANNELS"))
        return message.channel.send(
          `I cannot manage the channel. I need Manage Channels permission`
        );
      let channel =
      ...
      await channel.messages
        ...
        .then(async msgID => {
          ...
          if (!message.guild.me.hasPermission("MANAGE_ROLES"))
            return message.channel.send(
              `I cannot manage the role. I need Manage Roles permission`
            );
          let role =
            ...
            message.guild.roles.cache.find(
              r =>
                ...
                args
                  .slice(
                    args[0].length + 1 + args[1].length + 1 + args[2].length + 1
                  )
                  .join(" ")
            );
          ...
          if (!message.guild.me.hasPermission("ADD_REACTIONS"))
            return message.channel.send(
              `I cannot react the message. I need Add Reactions permission`
            );
          msgID.react(emoji);
          ...
          if (!message.guild.me.hasPermission("EMBED_LINKS")) {
            return await message.channel
              .send(
                `I cannot send embedded message. I will send in a normal message.`
              )
              .then(() => {
                message.channel.send(
    ...
    case "crembed":
      ...
      if (!args[3])
        return message.channel.send(
          "Please provide a embed title and a description."
        );
      let titleNdescription = args
        .join(" ")
        .slice(args[0].length + 1 + args[1].length + 1 + args[2].length + 1)
        .split(" | ");

      let msgID_Embed = await channelEmbed.send({
        embed: {
          title: titleNdescription[0],
          description: titleNdescription[1],
          color: "#19ce0e"
        }
      });
      ...
      if (!reactionsEmbed[channelEmbed.id]) {
        ...
        if (!reactionsEmbed[channelEmbed.id][msgID_Embed.id]) {
          ...
          reactionsEmbed[channelEmbed.id][msgID_Embed.id][emojiEmbed] =
            roleEmbed.name;
        }
      }
      ...
      fs.writeFileSync(
        "./config/reactions.json",
        JSON.stringify(reactionsEmbed, null, 4),
        err => {}
      );
